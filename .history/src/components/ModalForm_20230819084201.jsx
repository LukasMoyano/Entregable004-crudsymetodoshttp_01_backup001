...
const ModalForm = ({
  ...
  // Utilizamos useForm de react-hook-form para manejar el formulario
  ...
  const submit = (data) => {
    ...
    if (isUserToUpdate) {
      // Si estamos actualizando un usuario, llamamos a updateUser
      updateUser(data, reset);
    } else {
      // Si estamos creando un nuevo usuario, llamamos a createUser
  ...
  // Efecto para cargar los datos del usuario a editar cuando isUserToUpdate cambia
  useEffect(() => {
    if (isUserToUpdate) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          type="submit"
          className="bg-[#555A88] text-white py-2 px-4 rounded-md hover:bg-[#666985]"
        >
...
ModalForm.propTypes = {
  ...
  DEFAULT_VALUES: PropTypes.object.isRequired,
};
